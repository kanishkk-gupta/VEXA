...
> **Current status: Milestone 3.5 — Full Agent Pipeline & IDE Frontend**
> VEXA now features a fully operational multi-agent pipeline (M3) powered by CrewAI, executing the Sequential architecture to analyze, code, test, and verify software requirements. It also includes a robust React/Vite Frontend Dashboard featuring real-time Observability, an integrated Monaco Code Editor, and a Live Web Preview server!
...
| Observability / tracing | M8 | ❌ Deferred |
| Evaluation framework | M8 | ❌ Deferred |
| Benchmark runner (50 tasks) | M9 | ❌ Deferred |
| PostgreSQL persistence | M4+ | ❌ Deferred |
| MCP integration | M3+ | ❌ Deferred |
| Memory layer | M5+ | ❌ Deferred |
| Safety guardrails | M6+ | ❌ Deferred |

---

## Milestone 2 — Tool Layer

VEXA includes a comprehensive tool layer designed for future agents to use securely.
...
- **Execution Sandbox**: Runs allowlisted commands (like `pytest` or `python`) inside the workspace with captured stdout/stderr, strict timeouts, and memory limits. Currently defaults to `LocalSandboxExecutor`.
- **API Endpoints**: All tool capabilities are exposed via the `/workspaces/` API, operating only on `workspace_id`s, not host paths.

---

## Milestone 3 & 3.5 — Agents & IDE Frontend

VEXA now integrates a complete **CrewAI** multi-agent pipeline and a rich developer dashboard.

- **Agent Pipeline**: 
  - **RequirementAnalyst**: Breaks down natural language requests.
  - **Planner**: Designs architecture and creates implementation plans.
  - **Coder**: Implements the logic and uses filesystem tools to edit code.
  - **Tester**: Writes tests and executes the sandbox engine to verify code.
  - **Debugger**: Iteratively repairs failing tests autonomously.
  - **Verifier**: Audits the final code against the original acceptance criteria.
- **Resilient LLM Execution**: Custom failover logic safely handles rate limits (e.g. Mistral's 1-RPS limit) and API quota exhaustion (seamlessly swapping to fallback providers).
- **Frontend Dashboard**: A stunning React/Vite web interface that features:
  - **Live Observability**: Real-time agent status and log streaming.
  - **Integrated Code Editor**: A Monaco-powered IDE that lets you explore generated workspace code with rich syntax highlighting.
  - **Live Web Preview**: An integrated iframe to instantly preview HTML/JS applications generated by the agents, served securely through FastAPI.
  - **Dynamic LLM Configuration**: A built-in UI panel to select LLM models (e.g. `groq/openai/gpt-oss-120b`) and override primary/fallback API keys on a per-run basis without restarting the server.
- **Agent Optimizations**:
  - `ProjectAnalyst` is optimized with pre-injected workspace context to bypass redundant tool calls, drastically reducing execution time on existing projects.
